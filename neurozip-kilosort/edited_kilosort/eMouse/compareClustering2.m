...
testCluIDs = unique(cluTest); %seems like we are detecting more clusters-- makes sense for premerge (remember ks intentionally oversplits)
jitter = 12;

nSp = zeros(max(testCluIDs), 1); %number of spieks associated with each TEST cluster
for j = 1:max(testCluIDs);
    nSp(j) = max(1, sum(cluTest==j));
end
nSp0 = nSp;

for cGT = 1:length(GTcluIDs) %for each cluster ID (ground truth)
%     fprintf(1,'ground truth cluster ID = %d (%d spikes)\n', GTcluIDs(cGT), sum(cluGT==GTcluIDs(cGT)));
    
    rGT = int32(resGT(cluGT==GTcluIDs(cGT))); %where in resGT do spikes belong to cGT
...
%     S = sparse(numel(rGT), max(testCluIDs));
    ...
    for j = 1:numel(cluTest)
        ...
        if resTest(j)>rGT(igt)-jitter %if cluster of spike j's id is in range of rGT, we have a hit 
...
%         mergeIDs(1), scores(1), sum(cluTest==mergeIDs(1)), fps(best), misses(best));
    ...
    S0 = S(:, best);
    nSp = nSp + nSp0(best); %TODO THIS IS WHERE MERGING STUFF HAPPENS-- KEEPS IF BENEFICIAL, DISCARDS IF NOT-- SHOULD I KEEP THIS?? DOES IT DEPEND ON GT KNOWLEDGE??
...
numMerges = zeros(1, length(GTcluIDs)); %TODO KEEP AN EYE ON THIS
fprintf(1, '\n\n--Results Summary--\n')
