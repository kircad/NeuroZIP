...
for ibatch = 1:Nbatch    
    ...
    if ops.GPU %st = spike times, id = where spikes are, x = amplitudes, PCproj = projection onto every (?) template (basically each spike has a 1xnumTemp array- not totally sure what it represents (maybe related to clusterid?))
        [st, id, x, errC, PCproj] ...
