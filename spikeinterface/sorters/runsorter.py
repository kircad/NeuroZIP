...
if __name__ == '__main__':
    ...
    run_time, sorting = run_sorter_local(
...
"""
    (parent_folder / "in_container_sorter_script.py").write_text(py_script, encoding="utf8") # TODO SAVE RUNTIME INFO AS PART OF SCRPT HERE - WHY ISNT IT SAVING?
    volumes = {}
    ...
    if run_error:
        ...
        if with_output:
            try:
                ...
                with (folder / 'spikeinterface_log.json').open("r", encoding="utf8") as f:
                    data = json.load(f)
...
def read_sorter_folder(folder, register_recording=True, sorting_info=True, raise_error=True):
    ...
    run_time = read_runtime_from_file(f'{folder}/run_time.txt')
    return run_time, sorting
