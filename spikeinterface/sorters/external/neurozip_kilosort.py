...
def check_if_installed(neurozip_kilosort_path: Union[str, None]):
    if neurozip_kilosort_path is None:
        return False
    assert isinstance(neurozip_kilosort_path, str)

    if neurozip_kilosort_path.startswith('"'):
        neurozip_kilosort_path = neurozip_kilosort_path[1:-1]
    neurozip_kilosort_path = str(Path(neurozip_kilosort_path).absolute())

    if (Path(neurozip_kilosort_path) / 'edited_kilosort' / "preprocessData.m").is_file():
        return True
    else:
        return False


class NeuroZIP_KilosortSorter(KilosortBase, BaseSorter):
    """Kilosort Sorter object."""

    sorter_name: str = "neurozip_kilosort"
    compiled_name: str = "nzks_compiled" # TODO CHANGE THIS?
    neurozip_kilosort_path: Union[str, None] = os.getenv("NEUROZIP-KILOSORT_PATH", None)
    ...
    environment variables or using NeuroZIP_KilosortSorter.set_kilosort_path().\n\n
    ...
    def is_installed(cls):
        if cls.check_compiled():
            return True
        return check_if_installed(cls.neurozip_kilosort_path)

    @classmethod
    def get_sorter_version(cls):
        if cls.check_compiled():
            return "compiled"
        commit = get_git_commit(os.getenv("NEUROZIP-KILOSORT_PATH", None))
        if commit is None:
            return "unknown"
        else:
            return "git-" + commit
    ...
    @classmethod
    def set_nzkilosort_path(cls, neurozip_kilosort_path: str):
        neurozip_kilosort_path = str(Path(neurozip_kilosort_path).absolute())
        NeuroZIP_KilosortSorter.neurozip_kilosort_path = neurozip_kilosort_path
        try:
            print("Setting NEUROZIP-KILOSORT_PATH environment variable for subprocess calls to:", neurozip_kilosort_path)
            os.environ["NEUROZIP-KILOSORT_PATH"] = neurozip_kilosort_path
        except Exception as e:
            print("Could not set NEUROZIP-KILOSORT_PATH environment variable:", e)
    ...
    def _check_params(cls, recording, output_folder, params):
        ...
        if p["Nfilt"] is None:
            p["Nfilt"] = (nchan // 32) * 32 * 4
        else:
            p["Nfilt"] = p["Nfilt"] // 32 * 32
        if p["Nfilt"] == 0:
            p["Nfilt"] = nchan * 8
        if p["NT"] is None:
            p["NT"] = 32 * 1024 + p["ntbuff"]
        else:
