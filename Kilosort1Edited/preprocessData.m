function [rez, DATA, uproj, compressionOps] = preprocessData(ops)
tic;
uproj = [];
...
if ~exist('DATA', 'var')
    DATA = zeros(NT, rez.ops.Nchan, Nbatch_buff, 'int16'); %WHY ALLOCATE THIS TO MEMORY???? TODO JUST USE FREAD LIKE IN KS3
end
...
while 1
    ...
    if ops.CAR
        dataRAW = dataRAW - median(dataRAW, 2); % subtract median across channels CAR
    end
...
if strcmp(ops.initialize, 'fromData')
    ...
    wPCA = ops.wPCA(ixt, 1:3); %get three template waveforms from previous data (how is this computed?)
...
for ibatch = 1:Nbatch
    ...
    datr    = datr * Wrot; %APPLY WHITENING MATRIX
    ...
    if strcmp(ops.initialize, 'fromData') %&& rem(ibatch, 10)==1
        ...
        [row, col, mu] = isolated_peaks(dataRAW, ops.loc_range, ops.long_range, ops.spkTh); %uses simple threshold
        rez.batchSpikes = vertcat(rez.batchSpikes, [ibatch length(row)]);
        % find their PC projections
        uS = get_PCproj(dataRAW, row, col, wPCA, ops.maskMaxChannels);
        
        uS = permute(uS, [2 1 3]);
        uS = reshape(uS,numel(row), Nchan * size(wPCA,2)); %why all the permutations lol
    ...
    if ibatch<=Nbatch_buff
        DATA(:,:,ibatch) = gather_try(datr); %WHAT IS THE POINT OF THIS?? NEEDLESS RAM USAGE?? TODO FIND WAY TO NOT DO THIS 
    end
