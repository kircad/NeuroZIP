function rez = fitTemplates(rez, DATA, uproj, batchesToUse)
...
freqUpdate = length(iperm);
iUpdate = 1:freqUpdate:Nbatch; %update params every length(iperm) iterations??


dbins = zeros(100, Nfilt);
dsum = 0;
miniorder = repmat(iperm, 1, ops.nfullpasses); %STOCHASTIC BATCH OPTIMIZATION HERE
...
%HAVE NO IDEA WHAT THIS SEGMENT DOES - CREATING VECTORS THAT CONTROL ANNEALING RATE (?) 
...
while (i<=length(miniorder)) %I thought loop proceeded until cost function converged??
    ...
    if i>1 &&  ismember(rem(i,Nbatch), iUpdate) %&& i>Nbatch
        ...
        if  ops.shuffle_clusters &&...
                i>Nbatch && rem(rem(i,Nbatch), freqUpdate)==1    % i<Nbatch*ops.nannealpasses
...
%             figure;
    ...
    if ops.verbose
        ...
        msg = sprintf('Time %2.2f, batch %d/%d, mu %2.2f, neg-err %2.6f, NTOT %d, n100 %d, n200 %d, n300 %d, n400 %d\n', ...
            toc, i,length(miniorder),nanmean(mu(:)), nanmean(delta), round(sum(nsort)), ...
...
%TODO visualize templates created for each batch?
if ~ops.GPU
