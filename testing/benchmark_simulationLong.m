function benchmark_simulationLong(rez, GTfilepath)
...
[allScores, allFPrates, allMissRates, allMerges] = ...
    compareClustering2(gtClu, gtRes, testClu, testRes, []); %TODO IS THIS WITH CURATION?? I AM SO CONFUSED
...
finalScores = cellfun(@(x) x(end), allScores); %x(1) is without merges (data-based -- different from KS merge algorithm (I think?), x(end) is with)
...
newCells = allScores(end-4:end);
fprintf('%d / %d good new cells, score > 0.8 (pre-merge) \n', sum(cellfun(@(x) x(1), newCells)>.8), numel(newCells))
fprintf('%d / %d good new cells, score > 0.8 (post-merge) \n', sum(cellfun(@(x) x(end), newCells)>.8), numel(newCells))
